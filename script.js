...
inputText.addEventListener("input", triggerAutoTranslate)
...
document.getElementById("generatePrompt").onclick = async function () {
    ...
    if (!prompt) return
    inputText.innerText = "AI is thinking..."
    ...
    try {
        ...
        inputText.innerText = "AI generation error"
    }
}
...
async function loadTemplates() {
    try {
        ...
        blocks.forEach(block => {
            ...
            const value = block.replace(match[0], "").trim()
            templates[key] = value
        })
